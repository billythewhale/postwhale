 ...
 */
export function formatRedisStreamId(input: string): string {
  if (!input || input.trim() === '') {
    return '0-0';
  }
  ...
  if (trimmedInput.includes('-')) {
    ...
    if (parts.length !== 2) {
      throw new Error(
        `Invalid Redis Stream ID format: ${input}. Expected format: timestamp-sequence`,
      );
    }
    ...
    if (timestamp.length > 13) {
      throw new Error(
        `Invalid Redis Stream ID format: ${input}. Timestamp cannot exceed 13 digits`,
      );
    }
